...
/* eval_p02_bat_life_when_discharging - Evaluate conservative battery life when discharging.
 * Use (maximum consuming current * update interval) as one factor.
 */
static int eval_p02_bat_life_when_discharging(
...
/* eval_p02_bat_life_when_charging - Evaluate conservative battery life when charging.
 * Use (maximum charging current * update interval) as one factor.
 */
static int eval_p02_bat_life_when_charging(
	int nowCap, int lastCap, int maxMah, int interval, int batCapMah)
{
	int bat_life = 0;
...
int AXC_Cap_Filter_P02_GetType(struct AXI_Cap_Filter *apCapFilter)
{
...
int AXC_Cap_Filter_P02_FilterCapacity(struct AXI_Cap_Filter *apCapFilter, int nowCap, int lastCap, bool hasCable, bool isCharing, bool isBatFull, bool isBatLow, int maxMah, int interval)
{
	int bat_life;
	AXC_Cap_Filter_P02 *this = container_of(apCapFilter, AXC_Cap_Filter_P02, parentCapFilter);

	/* the criteria to set P02 bat full*/
	if ((nowCap >= BAT_LIFE_BEFORE_FULL_ONE_UNIT) && (isBatFull)){
		bat_life = BAT_LIFE_FULL;
		pr_debug( "[BAT][Fil] %s(), set bat life 100\n", __func__);
		return bat_life;
	}
	...
	if (lastCap >= 88) {		
		if (hasCable) {	// has cable, use slow rise algo
			bat_life = eval_p02_bat_life_when_charging(nowCap, lastCap, maxMah, interval, this->capMah);
		} else {	// no cable, use slow down algo
			bat_life = eval_p02_bat_life_when_discharging(nowCap, lastCap, maxMah, interval, this->capMah);
		}
	//Eason P03 capacity may increase when dont have extChg+++
	}else if(false==hasCable){
		if(nowCap > lastCap)
		{
				bat_life = lastCap;
				printk("[BAT][Fil][P03]:noCable but cap increase, keep last cap\n");
		}else{
				bat_life = nowCap;
		}
	//Eason P03 capacity may increase when dont have extChg---
...
extern void AXC_Cap_Filter_P02_Constructor(AXI_Cap_Filter *apCapFilter, int filterType, int capMah)
{
	...
	this->parentCapFilter.getType = AXC_Cap_Filter_P02_GetType;
	this->parentCapFilter.filterCapacity = AXC_Cap_Filter_P02_FilterCapacity;
