...
static void bat_cmd_read_all_info(const char *msg, int index )
{
	...
	printk(DBGMSK_BAT_INFO "[BAT] phone bat volt from HW: %d \r\n", phone_bat_info->get_prop_batt_volt_byhw());
...
void PadDock_AC_PowerSupplyChange(void)
{
    power_supply_changed(&pad_ac_psy);
    power_supply_changed(&dock_ac_psy);
    printk("[BAT]PadDock AC PowerSupplyChange\n");
}
// when A66 Cap = 0% shutdown device no matter if has cable---
...
//Eason takeoff Battery shutdown +++    
    ...
    batLowWorkVoltage = phone_bat_info->get_prop_batt_volt_byhw();
//Eason takeoff Battery shutdown ---    
...
//ASUS_BSP Eason_Chang add event log +++
	ASUSEvtlog("[BAT] phone bat volt from HW: %d \r\n", batLowWorkVoltage);//Eason takeoff Battery shutdown
			     ...
			     void *ptr)
{
	...
	pr_debug( "[BAT] phone bat volt from HW: %d \r\n", phone_bat_info->get_prop_batt_volt_byhw());
