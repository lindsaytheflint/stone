...
static void cal_bat_capacity_work(struct work_struct *work)
{
	...
	*/
	...
	//if (one_time_curr > 0) {  // on discharging
        ...
        else if(true == WhenBootUpDoVf)//Eason: Do VF with Cable when boot up+++
        {
        	online = getIfonline();
...
int AXC_Gauge_A66_GetNextPollingInterval(struct AXI_Gauge *gauge)
{
	...
	//if (this->ocvPercent < OCV_PER_SPEEDUP_UPDATE)//Eason repoert capacity to gauge for judge interval
	if (ReportCapToGaugeForJudgeInterval() < OCV_PER_SPEEDUP_UPDATE)//Eason repoert capacity to gauge for judge interval
