...
struct pm8921_bms_chip *bms_chip;
//ASUS_BSP Eason notify bms change OCVtable---
				...
				struct pm8921_soc_params *raw)
{
	...
	pr_debug("0p625 = %duV\n", chip->xoadc_v0625);
	pr_debug("1p25 = %duV\n", chip->xoadc_v125);
	printk("last_good_ocv_raw= 0x%x, last_good_ocv_uv= %duV\n",
			raw->last_good_ocv_raw, raw->last_good_ocv_uv);
						...
						int *delta_time_s)
{
	...
	*fcc_uah = calculate_fcc_uah(chip, batt_temp, chargecycles);
	printk("FCC = %uuAh batt_temp = %d, cycles = %d\n",
	...
	*remaining_charge_uah = calculate_remaining_charge_uah(chip, raw,
	...
	printk("cc_uah = %duAh raw->cc = %x cc = %lld after subtracting %x\n",
				*cc_uah, raw->cc,
	...
	*unusable_charge_uah = calculate_unusable_charge_uah(chip, *rbatt,
	...
	//Eason force ER UUC=0+++
	pr_debug("[BAT][BMS]force ER UUC=0+++\n");
	*unusable_charge_uah = 0;
	pr_debug("[BAT][BMS]force ER UUC=0---:UUC = %uuAh\n",*unusable_charge_uah);
	//Eason force ER UUC=0---
}
...
//Eason may charge when device shutdown can not use shutdown_soc +++
#if 0
...
//Eason: take off some dummy info +++
#if 0
					...
					int batt_temp, int chargecycles)
{
	...
	int new_ucc_uah;
	int new_rbatt;
	//int shutdown_soc;//Eason may charge when device shutdown can not use shutdown_soc
	...
	//ASUS BSP: Eason check correct BMS RUC+++
	if( remaining_usable_charge_uah < 0)
	{
		gIsBMSerror = true;
		printk("[BAT][BMS]:error RUC<0\n");
	}
	//ASUS BSP: Eason check correct BMS RUC---
...
//ASUS_BSP Eason : calculate BMS when update capacity +++
//use A68_1035 pm8921_bms_get_percent_charge
#if 0
...
//ASUS_BSP Eason : calculate BMS when charging began+++ 
#if 0 
void pm8921_bms_charging_began(void)
{
	struct pm8921_soc_params raw;

	printk("[BAT][BMS]:pm8921_bms_charging_began+++\n");//ASUS_BSP Eason
	mutex_lock(&the_chip->last_ocv_uv_mutex);
	read_soc_params_raw(the_chip, &raw);
	mutex_unlock(&the_chip->last_ocv_uv_mutex);
	...
	printk("[BAT][BMS]:start_percent = %u%%\n", the_chip->start_percent);//ASUS_BSP Eason
	printk("[BAT][BMS]:pm8921_bms_charging_began---\n");//ASUS_BSP Eason
}
#endif
void pm8921_bms_charging_began(void)
{
	...
	the_chip->start_percent = calculate_state_of_charge(the_chip, &raw,
					batt_temp, last_chargecycles);//ASUS_BSP Eason : calculate BMS when charging began
	mutex_unlock(&the_chip->last_ocv_uv_mutex);
...
void pm8921_bms_charging_end(int is_battery_full)
{
	...
	struct pm8921_soc_params raw;
      ...
      printk("[BAT][BMS]:pm8921_bms_charging_end+++\n");//ASUS_BSP Eason
	if (the_chip == NULL)
		return;
	...
	//ASUS_BSP Eason:printk BMS cycle info+++
	printk("end_percent = %u%% last_charge_increase = %d"
			"last_chargecycles = %d\n",
...
//ASUS_BSP Eason notify bms change OCVtable+++
void notify_bms_changeOCVtable(void)
{
	bms_chip->pc_temp_ocv_lut = palladium_1500_data_TWS.pc_temp_ocv_lut;
    printk("[BAT][Bms]notify change TWS OCV table\n");
}
//ASUS_BSP Eason notify bms change OCVtable---
