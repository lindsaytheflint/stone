...
static int TestSynaTouch(struct i2c_client *client)
{

	int lnResult = I2C_TEST_PASS;
	int error;

	i2c_log_in_test_case("TestSynaTouch++\n");
	error = i2c_check_functionality(client->adapter, I2C_FUNC_I2C);
	if (!error) {
		i2c_log_in_test_case("TestSynaTouch failed\n");
...
{
	__I2C_STRESS_TEST_CASE_ATTR(TestSynaTouch),
};
#endif
//ASUS_BSP simpson: add for I2C_STRESS_TEST ---
