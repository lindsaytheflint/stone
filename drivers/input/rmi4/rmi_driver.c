...
#endif
#include <linux/gpio.h> //ASUS_BSP simpson: add for hardware reset ++
...
static ssize_t rmi_driver_hardreset_store(struct device *dev,
                                       struct device_attribute *attr,
                                       const char *buf, size_t count);
//ASUS_BSP simpson: add for hardware reset ---
...
//ASUS_BSP simpson: add for hardware reset +++
       __ATTR(hardreset, RMI_WO_ATTR,
              rmi_show_error, rmi_driver_hardreset_store),
//ASUS_BSP simpson: add for hardware reset ---
#if REGISTER_DEBUG
...
#include <linux/syscalls.h> //ASUS_BSP simpson: fix for CTS write permission rule ++
static ssize_t rmi_driver_enabled_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	...
	printk("Owner UID: %ld GID: %ld\n",  sys_getuid(), sys_getgid()); //ASUS_BSP simpson: fix for CTS write permission rule ++
	return count;
}
