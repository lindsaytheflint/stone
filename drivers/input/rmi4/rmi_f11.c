...
#else
#define F11_DEFAULT_LED_LOW F11_DEFAULT_LED_HI
#endif //F11_3LEVEL_LED
int led_level_now = 0;
int user_duration = F11_DEFAULT_LED_TIME;
int key_pressed_led_level = F11_DEFAULT_LED_HI;
int user_level_shift = 0;
static int finger_moving_led_level = F11_DEFAULT_LED_LOW;
static int basic_led_level = F11_DEFAULT_LED_BASIC;
static int unlock_duration = 10000;
static int user_mode = 0;
static int unlock1time = 0;
//ASUS_BSP simpson: add for keypad_bl ---
...
bool g_bIsSuspended = false;
//ASUS_BSP simpson: add for suspend/resume ---
extern bool g_bFoneInPad;
extern int g_resumeLog;
extern unsigned int g_XY;
...
int g_SYNA_2d_block = 0;
EXPORT_SYMBOL(g_SYNA_2d_block);
//ASUS_BSP simpson : add for 2d_block ---
#if F11_3LEVEL_LED
//ASUS_BSP simpson: add for finger moving trigger +++
static ssize_t f11_mv_led_show(struct device *dev,
				   struct device_attribute *attr, char *buf);

static ssize_t f11_mv_led_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count);
//ASUS_BSP simpson: add for finger moving trigger ---
static ssize_t f11_basic_led_show(struct device *dev,
				   struct device_attribute *attr, char *buf);

static ssize_t f11_basic_led_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count);

#endif //F11_3LEVEL_LED
//ASUS_BSP simpson: add for Customer Happiness +++
static ssize_t f11_key_led_show(struct device *dev,
				   struct device_attribute *attr, char *buf);

static ssize_t f11_key_led_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count);
static ssize_t f11_key_mode_show(struct device *dev,
				   struct device_attribute *attr, char *buf);

static ssize_t f11_key_mode_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count);
static ssize_t screen_unlocked_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count);
static ssize_t f11_key_led_level_shift_show(struct device *dev,
				   struct device_attribute *attr, char *buf);

static ssize_t f11_key_led_level_shift_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count);
//ASUS_BSP simpson: add for Customer Happiness ---
...
#if F11_3LEVEL_LED
//ASUS_BSP simpson: add for finger moving trigger +++
	__ATTR(mv_led, RMI_RW_ATTR, f11_mv_led_show, f11_mv_led_store),
//ASUS_BSP simpson: add for finger moving trigger ---
	__ATTR(basic_led, RMI_RW_ATTR, f11_basic_led_show, f11_basic_led_store),
#endif //F11_3LEVEL_LED
//ASUS_BSP simpson: add for Customer Happiness +++
	__ATTR(key_led, RMI_RW_ATTR, f11_key_led_show, f11_key_led_store),
	__ATTR(user_mode, RMI_RW_ATTR, f11_key_mode_show, f11_key_mode_store),
	__ATTR(screen_unlocked, RMI_WO_ATTR, rmi_show_error, screen_unlocked_store),
	__ATTR(level_shift, RMI_RW_ATTR, f11_key_led_level_shift_show, f11_key_led_level_shift_store),
//ASUS_BSP simpson: add for Customer Happiness ---
...
//ASUS_BSP simpson: report zero finger will cause error in some app++
...
EXPORT_SYMBOL(rmi_led_trigger_event);
void turnoff_keypad_bl(struct work_struct *work){
	rmi_led_trigger_event(keypad_led_trigger, basic_led_level);
}
EXPORT_SYMBOL(turnoff_keypad_bl);
//ASUS_BSP simpson: add for keypad_bl ---
//ASUS_BSP simpson: add for suspend/resume +++
#ifdef CONFIG_HAS_EARLYSUSPEND
...
static struct early_suspend rmi_early_suspend_desc = {
    ...
    .suspend = rmi_early_suspend,
    .resume = rmi_late_resume,
};
#endif
//ASUS_BSP simpson: add for suspend/resume ---
#define MAX_FINGER		10
#define REPORT_COUNT	200
...
//ASUS_BSP simpson: add for tracking fingers +++
	struct f11_2d_data *data = &sensor->data;
	int X,Y;
	static int finger_count[MAX_FINGER];
//ASUS_BSP simpson: add for tracking fingers ---
	...
	//ASUS_BSP simpson: add for tracking fingers +++
		X = ((data->abs_pos[i].x_msb << 4) |
			data->abs_pos[i].x_lsb);
		Y = ((data->abs_pos[i].y_msb << 4) |
			data->abs_pos[i].y_lsb);
	//ASUS_BSP simpson: add for tracking fingers ---
...
//ASUS_BSP simpson: add for finger moving trigger +++
		if (i == (sensor->nbr_fingers - 1)){
			if ((finger_pressed_count > 0) && (led_state ==0)){
			    if(delayed_work_pending(&keypad_off_work)) {
			        cancel_delayed_work_sync(&keypad_off_work);
			    }
				rmi_led_trigger_event(keypad_led_trigger, finger_moving_led_level);
...
//ASUS_BSP simpson: add for finger moving trigger ---
	...
	g_SYNA_2d_block = finger_pressed_count;	//ASUS_BSP simpson: add for 2d_block ++
...
//ASUS_BSP simpson: add for creating virtual_key_maps +++
#define MAX_LEN		200
...
keypad_bl_workqueue = create_singlethread_workqueue("keypad_bl_wq");
INIT_DELAYED_WORK(&keypad_off_work, turnoff_keypad_bl);
...
static ssize_t f11_mv_led_show(struct device *dev,
				   ...
				   char *buf)
{
	return snprintf(buf, PAGE_SIZE, "finger_moving_led_level=%d\n",finger_moving_led_level);
}
...
static ssize_t f11_mv_led_store(struct device *dev,
				    ...
				    size_t count)
{
	if (sscanf(buf, "%d", &finger_moving_led_level) != 1)
		return -EINVAL;
	if (finger_moving_led_level > F11_LED_MAX_LEVEL)
		finger_moving_led_level = F11_LED_MAX_LEVEL;
	return count;
}
//ASUS_BSP simpson: add for finger moving trigger ---
static ssize_t f11_basic_led_show(struct device *dev,
				   ...
				   char *buf)
{
	return snprintf(buf, PAGE_SIZE, "basic_led_level=%d\n",basic_led_level);
}
...
static ssize_t f11_basic_led_store(struct device *dev,
				    ...
				    size_t count)
{
	if (sscanf(buf, "%d", &basic_led_level) != 1)
		return -EINVAL;
	if (basic_led_level > F11_LED_MAX_LEVEL)
		basic_led_level = F11_LED_MAX_LEVEL;
	//force update! ++
	if(!delayed_work_pending(&keypad_off_work)) rmi_led_trigger_event(keypad_led_trigger, basic_led_level); //not pending means stay basic_led_level
	//force update! --
...
static ssize_t f11_key_led_show(struct device *dev,
				   ...
				   char *buf)
{
	return snprintf(buf, PAGE_SIZE, "key_pressed_led_level=%d\n", key_pressed_led_level);
}
...
static ssize_t f11_key_led_store(struct device *dev,
				    ...
				    size_t count)
{
	int finger_moving_led_level_old = finger_moving_led_level;

	if (sscanf(buf, "%d", &key_pressed_led_level) != 1)
		return -EINVAL;
	if (key_pressed_led_level > F11_LED_MAX_LEVEL)
		key_pressed_led_level = F11_LED_MAX_LEVEL;
#if F11_3LEVEL_LED
	finger_moving_led_level = key_pressed_led_level - F11_DEFAULT_LED_DIFF;
	if (finger_moving_led_level < 0) finger_moving_led_level = 0;
#else
	finger_moving_led_level = key_pressed_led_level;
#endif //F11_3LEVEL_LED
	//force update! ++
	if (basic_led_level > 0 && basic_led_level == finger_moving_led_level_old) {	//means always on & follow LED_LOW, update immediately
		basic_led_level = finger_moving_led_level;
		rmi_led_trigger_event(keypad_led_trigger, basic_led_level);
	}
	if(led_level_now) rmi_led_trigger_event(keypad_led_trigger, key_pressed_led_level); //pending means key pressed, update immediately
	//force update! --
				    ...
				    size_t count)
{
	if (sscanf(buf, "%d", &user_mode) != 1)
		return -EINVAL;
	if (user_mode >= (F11_LED_MAX_TIME*-1) && user_mode <= -100) {	//manually setting turn on duration(100ms~F11_LED_MAX_TIME) with basic_led_level=1
		user_duration = user_mode*-1;
		basic_led_level = 1;
	} else if (user_mode > -100 && user_mode < 0) {                 //native pre-defined mode(-1~-99) means always on
		basic_led_level = finger_moving_led_level;
	} else if (user_mode == 0) {					//pre-defined mode(0~99). mode 0, turn-on 2s (default)
		user_duration = F11_DEFAULT_LED_TIME;
		basic_led_level = F11_DEFAULT_LED_BASIC;
	} else if (user_mode == 1) {					//mode 1, turn-on 10s
		user_duration = 10000;
		basic_led_level = F11_DEFAULT_LED_BASIC;
	} else if (user_mode == 2) {					//mode 2, always on
		basic_led_level = finger_moving_led_level;
	} else if (user_mode == 3) {					//mode 3, always off
		basic_led_level = F11_DEFAULT_LED_BASIC;
	} else if (user_mode >= 100 && user_mode <= F11_LED_MAX_TIME) {	//manually setting turn on duration(100ms~F11_LED_MAX_TIME)
		user_duration = user_mode;
		basic_led_level = F11_DEFAULT_LED_BASIC;
	...
	//force update! ++
	if (basic_led_level == finger_moving_led_level) {	//always on/off
		rmi_led_trigger_event(keypad_led_trigger, basic_led_level);
	} else {	//reschedule to new user_duration
		if(delayed_work_pending(&keypad_off_work)) {
			cancel_delayed_work_sync(&keypad_off_work);
		}
		queue_delayed_work(keypad_bl_workqueue, &keypad_off_work, msecs_to_jiffies(user_duration));
...
static ssize_t f11_key_led_level_shift_show(struct device *dev,
				   ...
				   char *buf)
{
	return snprintf(buf, PAGE_SIZE, "user_level_shift=%d\n", user_level_shift);
}
...
static ssize_t f11_key_led_level_shift_store(struct device *dev,
				    ...
				    size_t count)
{
	if (sscanf(buf, "%d", &user_level_shift) != 1)
		return -EINVAL;
	if (user_level_shift > finger_moving_led_level)
		user_level_shift = finger_moving_led_level;
