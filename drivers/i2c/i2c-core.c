...
//ASUS_BSP +++ Peter Lu "For wait microp control lightsensor issue"
...
int i2c_transfer(struct i2c_adapter *adap, struct i2c_msg *msgs, int num)
{
	...
	if (adap->algo->master_xfer) {
		...
		/*ASUS_BSP +++ Peter Lu "Delay for wait microp ready"*/
		if ( msgs[0].addr == 0x15 && ( msgs[0].buf[0] == 0x10 ||msgs[0].buf[0] == 0x11 ||
							msgs[0].buf[0] == 0x12 ||msgs[0].buf[0] == 0x13 ||msgs[0].buf[0] == 0x14)){
			i2c_debug(DEBUG_TRACE , 3, "Delay for wait microp ready,Address = 0x%x, Cmd = 0x%x\r\n", msgs[0].addr, msgs[0].buf[0]);
			mdelay(12);
			if ( ret < 0 )
				mdelay(15);
		}
