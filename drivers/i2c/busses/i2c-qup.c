...
static unsigned int qup_bus_suspended[APQ_8064_GSBI5_QUP_I2C_BUS_ID+1]={0};
...
unsigned int qup_check_suspended_flag(int i2c_index)
{
	if (i2c_index <= APQ_8064_GSBI5_QUP_I2C_BUS_ID)
		return qup_bus_suspended[i2c_index];
	else
		return 0;
}
...
void SiiMhlTxHwReset(unsigned short hwResetPeriod,unsigned short hwResetDelay);
void SiiMhlTxInitialize(unsigned char pollIntervalMs );
//ASUS_BSP--- for reset/re-init MHL TX 
...
static void msm_gsbi4_io_w(u32 data, void __iomem *addr)
{
	printk("msm_gsbi4_io_w %s: %08x %08x\n", __func__, (int) (addr), (data));
	writel_relaxed((data), (addr));
}

u32 msm_gsbi4_io_r(void __iomem *addr)
{
...
//ASUS_BSP TomChu : set GSBI1 i2c clock  +++
struct qup_i2c_dev *g_gsbi1_dev=NULL;
int set_gsbi1_i2c_clk(int clk_freq)
{
	struct qup_i2c_dev *dev = g_gsbi1_dev;
	if (clk_freq <= 0 ||
			clk_freq > 400000) {
		return -1;
	}
	printk("%s: %d \n", __FUNCTION__, clk_freq);
	dev->pdata->clk_freq = clk_freq;
	dev->clk_ctl = 0;
...
//ASUS_BSP larry lai : GSBI4 porting +++	
	struct resource         *ctl_mem, *rw_mem;	
//ASUS_BSP larry lai : GSBI4 porting ---
	struct clk         *clk, *pclk;
	int ret = 0;
	int i;
...
//ASUS BSP mini-porting +++ GSBI4 porting
	if (pdata->use_gsbi_mux_mode == 4) {
		printk("use_gsbi_mux_mode 4 ++ \n");		
		ctl_mem = platform_get_resource_byname(pdev,
		...
		rw_mem = platform_get_resource_byname(pdev,
		...
		ctl_base = ioremap(ctl_mem->start,
			resource_size(ctl_mem));
		if (!ctl_base) {
			ret = -ENOMEM;
			goto get_res_failed;
		}
		rw_base = ioremap(rw_mem->start,
			resource_size(rw_mem));
		if (!rw_base) {
			ret = -ENOMEM;
			iounmap(ctl_base);
			goto get_res_failed;
		}
		val = msm_gsbi4_io_r(rw_base);
		msm_gsbi4_io_w((val | 0x200), rw_base);
		val = msm_gsbi4_io_r(rw_base);
		printk("qup_i2c_probe :  rw_base value is 0x%x\n", val);

		val = msm_gsbi4_io_r(ctl_base);
		msm_gsbi4_io_w(((val | 0x2) & ~(0x1)), ctl_base);
		val = msm_gsbi4_io_r(ctl_base);
		printk("qup_i2c_probe : ctl_base value is 0x%x\n", val);
		printk("use_gsbi_mux_mode 4 -- \n");		
	}	
//ASUS BSP mini-porting --- GSBI4 porting
...
extern void AX_MicroP_Bus_Suspending(int susp);
...
//ASUS_BSP+++ check MHL power state for determine i2c suspend
	...
	if (APQ_8064_GSBI4_QUP_I2C_BUS_ID == dev->adapter.nr) 
	{
		if (!qup_bus_suspended[dev->adapter.nr])
		{
	            printk("[i2c resume] no use to prepare qup i2c resume \r\n");	
			ASUSEvtlog("[i2c resume] no use to prepare qup i2c resume\n");								
			goto SKIP_QUP_SUSPEND_PREPARE;   // no use to prepare qup i2c resume
		}
	}
//ASUS_BSP+++ check MHL power state for determine i2c suspend
