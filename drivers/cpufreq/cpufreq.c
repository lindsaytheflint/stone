...
//#include "../../arch/arm/mach-msm/board-8064-gpu.c"
...
#ifdef CONFIG_CPU_PREVENT_MPDECISION
//LF: Following 2 structures
...
extern ssize_t acpuclk_set_vdd2(char *buf);
...
static ssize_t store_UV_mV_table(struct cpufreq_policy *policy, const char *buf, size_t count) {
	//acpuclk_set_vdd2(buf);
	//store_vdd_levels(cpufreq_policy.
...
#ifndef CONFIG_CPU_PREVENT_MPDECISION //LF: Disabled to prevent MPDecision
...
static ssize_t store_gpu_oc(struct cpufreq_policy *policy, const char *buf, size_t size)
{
	int ocval;
	sscanf(buf, "%d\n", &ocval);
	//kgsl_3d0_setmaxfreq(ocval);
	//kgsl_3d0_pdata.pwrlevel[0].gpu_freq=ocval;
	pr_info("[stone] gpu top speed changed from %d to %d\n",kgsl_3d0_getmaxfreq(),ocval);
	return size;
}
define_one_global_rw(gpu_oc);
...
#ifdef CONFIG_GPU_SYSFS
static struct attribute *gpu_oc_attrs[] = {
	&gpu_oc.attr,
	NULL
};

static struct attribute_group gpu_oc_attr_group = {
	.attrs = gpu_oc_attrs,
	.name = "gpu_oc",
};
#endif
...
#endif	/* CONFIG_CPU_VOLTAGE_TABLE */
#ifdef CONFIG_GPU_SYSFS
	rc = sysfs_create_group(cpufreq_global_kobject, &gpu_oc_attr_group);
#endif	
	return 0;
}
