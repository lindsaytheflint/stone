...
#include "search.c"
#include "symsearch.h"
SYMSEARCH_DECLARE_FUNCTION_STATIC(unsigned long, nr_running_s, void);
...
/*	busy_time = cputime64_add(kcpustat_cpu(cpu).cpustat[CPUTIME_USER], //LF: kstat_cpu(cpu).cpustat.user
			  kcpustat_cpu(cpu).cpustat[CPUTIME_SYSTEM]); //LF: kstat_cpu(cpu).cpustat.system
	busy_time = cputime64_add(busy_time, kcpustat_cpu(cpu).cpustat[CPUTIME_IRQ]); //LF: kstat_cpu(cpu).cpustat.irq);
	busy_time = cputime64_add(busy_time, kcpustat_cpu(cpu).cpustat[CPUTIME_SOFTIRQ]); //LF: kstat_cpu(cpu).cpustat.softirq);
	busy_time = cputime64_add(busy_time, kcpustat_cpu(cpu).cpustat[CPUTIME_STEAL]); //LF: kstat_cpu(cpu).cpustat.steal);
	busy_time = cputime64_add(busy_time, kcpustat_cpu(cpu).cpustat[CPUTIME_NICE]); //LF: kstat_cpu(cpu).cpustat.nice);
				      ...
				      const char *buf, size_t count)
{
	...
	for_each_online_cpu(j) {
		...
		if (dbs_tuners_ins.ignore_nice)
			dbs_info->prev_cpu_nice = kcpustat_cpu(j).cpustat[CPUTIME_NICE]; //LF: kstat_cpu(j).cpustat.nice;
	}
	return count;
}
	...
	for_each_cpu(j, policy->cpus) {
		...
		if (dbs_tuners_ins.ignore_nice) {
			...
			cur_nice = cputime64_sub(kcpustat_cpu(j).cpustat[CPUTIME_NICE],
						 j_dbs_info->prev_cpu_nice); //LF: kstat_cpu(j).cpustat.nice
			/*
			...
			j_dbs_info->prev_cpu_nice = kcpustat_cpu(j).cpustat[CPUTIME_NICE]; //LF: kstat_cpu(j).cpustat.nice;
		...
		for_each_cpu(j, policy->cpus) {
			...
			if (dbs_tuners_ins.ignore_nice) {
				j_dbs_info->prev_cpu_nice =
					kcpustat_cpu(j).cpustat[CPUTIME_NICE]; //LF: kstat_cpu(j).cpustat.nice;
			}
		}
...
{
	SYMSEARCH_BIND_FUNCTION_TO(pegasusq, cpu_up, cpu_up_s);
	SYMSEARCH_BIND_FUNCTION_TO(pegasusq, nr_running, nr_running_s);
	int ret;
