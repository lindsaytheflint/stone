...
#include "linux/p03_scaler.h"
#include "linux/al3010.h"

static struct i2c_client *p03_scaler_update_client;

extern int isMicroPConnected(void);
extern int AX_MicroP_readBattCapacity(int target);
extern bool read_scaler_version(unsigned char *ver);
...
extern unsigned char g_scaler_ver;
...
static int scaler_i2c_read(u8 addr, int len, void *data)
{
	...
	struct i2c_msg msg[] = {
		{
			.addr = p03_scaler_update_client->addr,
			.flags = 0,
			.len = 1,
			.buf = &addr,
		},
	...
	do{    
		status = i2c_transfer(p03_scaler_update_client->adapter, msg, ARRAY_SIZE(msg));
		if ((status < 0) && (i < retries)){
			msleep(5);
			printk("[p03_scaler_update] %s retry %d\r\n", __FUNCTION__, i);                                
			i++;
		}
	} while ((status < 0) && (i < retries));
        
	if(status < 0){
		printk("[p03_scaler_update] i2c read error %d\n", status);
	}
...
static int scaler_i2c_write(u8 addr, int len, void *data)
{
	...
	if (status < 0) {
		printk("[p03_scaler_update] i2c write error %d \n", status);
	}
...
bool SerialFlashWriteCode(unsigned char ucBankIndex)
{
	...
	printk("[p03_scaler_update] SerialFlashWriteCode ucBankIndex = %d\n", ucBankIndex);
	//ISP_EN
	byBuf[0] = 0x94;
	byBuf[1] = 0x6F;
	byBuf[2] = 0x80;
...
bool SerialFlash_ISP_Flow(void)
{
	...
	while(ucCount < _ENTER_ISP_COUNT)
	{
		...
		if(SerialFlashIntoISPMode()){
			printk("[p03_scaler_update] SerialFlashIntoISPMode success\n");
			break;
		}
	}

    if(ucCount >= _ENTER_ISP_COUNT){
        printk("[p03_scaler_update] SerialFlashIntoISPMode fail\n");
        return false;
    }
	

	//Set Default Value to some MCU Regs which may be modified by FW
	printk("[p03_scaler_update] SetDefaultValueToReg\n");
	if(!SetDefaultValueToReg())
	{
		printk("[p03_scaler_update] SetDefaultValueToReg fail\n");
		return false;
	}

    //Disable SerialFlash Hardware WriteProtection
    printk("[p03_scaler_update] DisableFlashHardwareWriteProtect\n");
	DisableFlashHardwareWriteProtect();

	old_fs = get_fs();
	set_fs(KERNEL_DS);
	strcpy(firmware_file_name_bin, "/system/etc/firmware/pad/A68-PAD-scaler.bin");
	
	fp = filp_open(firmware_file_name_bin, O_RDONLY, 0);
	if(IS_ERR_OR_NULL(fp)){
		printk("[p03_scaler_update] SerialFlash_ISP_Flow open (%s) fail\n", firmware_file_name_bin);
		return false;
	}
		
	buf = kmalloc(128*1024, GFP_KERNEL);
	if(buf==NULL) {
		printk("[p03_scaler_update] allocate mem for file fail\n");
		set_fs(old_fs);
		filp_close(fp, NULL);
		return false;
	}
	...
	if(file_len < 0) {
		printk("[p03_scaler_update]read file error\n");
		set_fs(old_fs);
		filp_close(fp, NULL);
		kfree(buf);
		return false;
	...
	//Block Erase
	if(!BlockErase(1)){
		printk("[p03_scaler_update] BlockErase 1 fail\n");
		return false;
	}
	...
	if (nReadBankCRC != 0xDE){
		printk("[p03_scaler_update] Check Block Erase 1 fail, nReadBankCRC = 0x%x\n", nReadBankCRC);
		return false;
	}
		
	//Block Erase
	if(!BlockErase(0)){
		printk("[p03_scaler_update] BlockErase 0 fail\n");
		return false;
	}
	...
	if (nReadBankCRC != 0xDE){
		printk("[p03_scaler_update] Check Block Erase 0 fail, nReadBankCRC = 0x%x\n", nReadBankCRC);
		return false;
	}
	...
	{
		printk("[p03_scaler_update] write scaler firmware fail!!!\n");
		printk("[p03_scaler_update] nCalculateCRC 1 = 0x%x\n", nCalculateCRC);
		printk("[p03_scaler_update] nReadCRC 1 = 0x%x\n", nReadCRC);
		return false;
	}
	...
	{
		printk("[p03_scaler_update] write scaler firmware fail!!!\n");
		printk("[p03_scaler_update] nCalculateCRC 0 = 0x%x\n", nCalculateCRC);
		printk("[p03_scaler_update] nReadCRC 0 = 0x%x\n", nReadCRC);
		return false;
	}
...
int HexString2Value(unsigned char* pData, unsigned char byCount)
{
	...
	printk("[p03_scaler_update] HexString2Value: pData = %s\n", pData);
	printk("[p03_scaler_update] HexString2Value: byCount = %d\n", byCount);
...
static long scaler_ioctl(struct file *file, unsigned cmd, unsigned long arg)
{
	...
	printk("[p03_scaler_update] scaler_ioctl +++\r\n");
	...
	switch (cmd) {
		case ASUS_SCALER_FW_UPDATE:
			printk("[p03_scaler_update] ioctl: ASUS_SCALER_FW_UPDATE ++\r\n");
			...
			if(pad_cap > 2) {
				...
				printk("[p03_scaler_update] ioctl: ASUS_SCALER_FW_UPDATE pad battery capacity is less than 2\r\n");
				ret = -EINVAL;
			}
			...
			pr_info("[p03_scaler_update] ioctl: ASUS_SCALER_FW_UPDATE --\r\n");
			break;
		case ASUS_SCALER_GET_FW_VERSION:
			printk("[p03_scaler_update] ioctl: ASUS_SCALER_GET_FW_VERSION ++\r\n");
			set_gsbi1_i2c_clk(100000);
...
static int scaler_open(struct inode *inode, struct file *file)
{
	...
	printk("[p03_scaler_update] scaler_open+++\n");
	return ret;
}
...
static int scaler_release(struct inode *inode, struct file *file)
{
	...
	printk("[p03_scaler_update] scaler_release+++\n");
	return ret;
}

static int p03_scaler_update_probe(struct i2c_client *client,
            const struct i2c_device_id *id)
{
 	printk("[p03_scaler_update] p03_scaler_update_probe\n");
	p03_scaler_update_client = client;

    return 0;
}

static const struct i2c_device_id p03_scaler_update_id[] = {
    { "p03_scaler_update", 0 },
    { }
};

static struct i2c_driver p03_scaler_update_i2c_driver = {
    .driver = {
        .name   = "p03_scaler_update",
    },
    .id_table   = p03_scaler_update_id,
    .probe      = p03_scaler_update_probe,
...
static int __init p03_scaler_update_init(void)
{
	int rc;
    struct device *class_dev;
    
    printk("[p03_scaler_update] p03_scaler_update_init\n");
    ...
    return i2c_add_driver(&p03_scaler_update_i2c_driver);
...
static void __exit p03_scaler_update_exit(void)
{
    printk("[p03_scaler_update] p03_scaler_update_exit\n");
    i2c_del_driver(&p03_scaler_update_i2c_driver);
}

module_init(p03_scaler_update_init);
module_exit(p03_scaler_update_exit);

MODULE_DESCRIPTION("p03 scaler update driver");
MODULE_LICENSE("GPL");
