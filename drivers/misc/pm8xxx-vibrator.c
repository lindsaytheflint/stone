...
#define VIBRATOR_PRINTK(mask, message, ...) \
	do { \
		if ((mask) & debug_mask) \
...
//ASUS BSP HANS++
		if (value >=10000)
		{
			vib_dev->level = 30;
			value = value - 10000;
			VIBRATOR_PRINTK(DEBUG_ENABLE,"[%s] Voltage=3V  msec=%d\n",__func__,value);
		}
		else
		{
			if (value % 2)
			...
			{
				vib_dev->level = 20;
				VIBRATOR_PRINTK(DEBUG_ENABLE,"[%s] Voltage=2V  msec=%d\n",__func__,value);
			}
		}
//ASUS BSP HANS--
...
static int mp_event_report(struct notifier_block *this, unsigned long event, void *ptr)
{
        switch (event)
	{
		case P01_ADD:
		{
			VIBRATOR_PRINTK(DEBUG_PAD_EVENT,"[%s] PAD ADD!!\n",__func__);
...
{
	unregister_microp_notifier(&mp_notifier);  //ASUS BSP HANS+
