...
#if 0        
        {"test_mode",            0x60,  4,  E_READWRITE},          //MICROP_TEST_MODE
#endif        
};
...
int uP_nuvoton_read_reg(int cmd, void *data){
    ...
    if(cmd>=0 && cmd < ARRAY_SIZE(uP_CMD_Table)){
        if(E_WRITE==uP_CMD_Table[cmd].rw 
            ...
            || (E_MODE_BOTH!=uP_CMD_Table[cmd].mode && g_curr_uP_mode!=uP_CMD_Table[cmd].mode)){ // skip read for these command
...
// Dock-Related
#define IsDockHavingEvt()                   ( ((reg_intr_sta>>INTR_STA_DOCK_AP_WAKE) & 0x1)?1:0 )
...
#define IsP01PwrBtnEvt()             ( ((reg_intr_sta>>INTR_STA_PWR_BTN) & 0x1)?1:0 )
#define IsP01HavingLSEvt()                 ( ((reg_intr_sta>>INTR_STA_ALS_INT) & 0x1)?1:0 )
#define IsP01HavingBat5Percent()                 ( ((reg_intr_sta>>INTR_STA_IND_GAUGE_5P) & 0x1)?1:0 )
#define IsP01HavingProxAct()                 ( ((reg_intr_sta>>INTR_STA_CAP_PROX_ACT) & 0x1)?1:0 )
#define IsP01PwrBtnLongPressEvt()             ( ((reg_intr_sta>>INTR_STA_LONG_PRESS_PWRKEY) & 0x1)?1:0 )
#if 0
#define IsTestModeBitTriggered()             ( ((reg_intr_sta>>INTR_STA_IND_TEST_MODE) & 0x1)?1:0 )
#endif
...
// P01
#define IsP01ACUSBIn()                      ( ((reg_input>>IN_AC_USB_IN) & 0x1)?1:0 )
#define IsP01VolUpPressed()                ( ((reg_input>>IN_VOL_UP_R) & 0x1)?0:1 )
#define IsP01VolDnPressed()                ( ((reg_input>>IN_VOL_DOWN_R) & 0x1)?0:1 )
#define IsP01PwrBtnPressed()                ( ((reg_input>>IN_PWR_BTN_R) & 0x1)?0:1 )
#define IsHW_MHL_H()                               ( ((reg_input>>IN_LCD_ID) & 0x1)?1:0 )
#define IsHW_MHL_L()                               ( ((reg_input>>IN_LCD_ID) & 0x1)?0:1 )
#define IsP01ProxMoveNear()                  ( ((reg_input>>IN_CAP_RPOX_INT_R) & 0x1)?0:1 )
...
// ASUS_BSP Jiunhau support usb host +++
#define MICROP_ENABLE_BST		0
#define MICROP_DISABLE_BST		1

static int microP_set_bst_en(bool on);
static int microP_get_bst_en(void);

static int microP_set_bst_en(bool on)
{
	int ret = -1;

	ret = AX_MicroP_setGPIOOutputPin(OUT_uP_DOCK_BST_EN_R, on);
	if (ret < 0) {
		printk(KERN_INFO "[MicroP] fail to set DOCK_BST_EN! (%d)(%d)\n", on, ret);
	} else {
		printk(KERN_INFO "[MicroP] set DOCK_BST_EN success (%d)\n", on);
	}
...
static int microP_get_bst_en(void)
{
	...
	pin_level = AX_MicroP_getGPIOOutputPinLevel(OUT_uP_DOCK_BST_EN_R);
	if (pin_level < 0) {
		printk(KERN_INFO "[EC] get DOCK_BST_EN status failed! (%d)\n", pin_level);
	} else {
		printk(KERN_INFO "[EC] get DOCK_BST_EN status success (%d)\n", pin_level);
	}
                                                                                    ...
                                                                                    if(IsDockPlugged()){
                                                                                               ...
                                                                                               //micropSendNotify(DOCK_PLUG_IN);
                                                                                                                                        ...
                                                                                                                                        if(g_A68_hwID == A68_SR2) {
                                                                                                                                            if(microP_get_bst_en() == MICROP_DISABLE_BST)
                                                                                                                                            {
                                                                                                                                                        printk(KERN_INFO "[MicroP] Enable boost\n");
                                                                                                                                                        microP_set_bst_en(MICROP_ENABLE_BST);
                                                                                                                                            }
                                                                                                                                        }
                                                                                                                                        // ASUS_BSP Jiunhau support usb host ---
                                                                                    }
                                                                                               ...
                                                                                               //micropSendNotify(DOCK_PLUG_OUT);
                                                                                                                                        // ASUS_BSP Jiunhau support usb host +++
                                                                                                                                        if(g_A68_hwID == A68_SR2) {
                                                                                                                                            if(microP_get_bst_en() == MICROP_ENABLE_BST)
                                                                                                                                            {
                                                                                                                                                    printk(KERN_INFO "[MicroP] Disable boost\n");
                                                                                                                                                    microP_set_bst_en(MICROP_DISABLE_BST);
                                                                                                                                            }
                                                                                                                                        }
                                                                                                                                        // ASUS_BSP Jiunhau support usb host ---
                                                                                    }
...
#if 0                                        
                                                                                if(IsTestModeBitTriggered()){
                                                                                    uP_nuvoton_read_reg(MICROP_TEST_MODE,&test_cnt);
                                                                                    printk("MicroP TestMode Count %d!!\r\n", test_cnt);
                                                                                }
#endif                                        
                                                               }                                 
                                        }
