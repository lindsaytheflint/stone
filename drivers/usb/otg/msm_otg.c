...
#include <linux/power_supply.h>
	...
	#define USB_FASTCHG_LOAD 1400 /* uA */
#endif 
...
static int __init msm_otg_probe(struct platform_device *pdev)
{
	...
	if (irq < 0) {
		dev_err(&pdev->dev, "%s: could not get FACTORY USB IRQ resource. "
	...
	if (ret < 0) {
		dev_err(&pdev->dev, "%s: FACTORY USB IRQ#%d request failed with error=%d"
