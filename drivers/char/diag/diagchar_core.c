...
#include <linux/delay.h> // ASUS_BSP+ "Stop Diag data from HSIC"
...
// ASUS_BSP+++ "Stop Diag data from HSIC"

//Event Report Control Request
static u8 DIAG_STOP_MSG1[] = {
   0x60, 0x00, 0x12, 0x6a, 0x7e
};

//Logging Config Request
static u8 DIAG_STOP_MSG2[] = {
   0x73, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xda, 0x81, 0x7e
};

//Extended Message Report Config Request
static u8 DIAG_STOP_MSG3[] = {
   0x7d, 0x5d, 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x74, 0x41, 0x7e
};

int diagchar_stop_hsic(void)
{
	int err = 0;
	int retry_cnt = 10;
	while (driver->in_busy_hsic_write == 1 && retry_cnt-- > 0) {
		msleep_interruptible(10);
	}
	pr_info("%s: SEND STOP MSG\n", __func__);
	driver->in_busy_hsic_write = 1;
	err = diag_bridge_write(DIAG_STOP_MSG1, sizeof(DIAG_STOP_MSG1));
	if (err) {
		pr_err("diag: mdm data on hsic write err: %d\n", err);
	}
	err = diag_bridge_write(DIAG_STOP_MSG2, sizeof(DIAG_STOP_MSG2));
	if (err) {
		pr_err("diag: mdm data on hsic write err: %d\n", err);
	}
	err = diag_bridge_write(DIAG_STOP_MSG3, sizeof(DIAG_STOP_MSG3));
	if (err) {
		pr_err("diag: mdm data on hsic write err: %d\n", err);
	}
...
static int diagchar_close(struct inode *inode, struct file *file)
{
	...
	*/
	...
	if (driver->logging_process_id == current->tgid) {
		diagchar_stop_hsic(); // ASUS_BSP+ "Stop Diag data from HSIC"
			   ...
			   unsigned int iocmd, unsigned long ioarg)
{
	...
	//ASUS_BSP + Yujoe "[A68][SS][N/A][fix]Fix "Diag_mdlog makes the device died"issue
	if((!(driver->hsic_device_enabled))&&(iocmd == DIAG_IOCTL_SWITCH_LOGGING)){
		//printk(KERN_INFO "diag_mdlog:diagchar_ioctl\n");
		return 2;
	}
	//ASUS_BSP + Yujoe "[A68][SS][N/A][fix]Fix "Diag_mdlog makes the device died"issue
