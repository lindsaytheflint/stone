...
//++++ ASUS_BSP: Louis
extern void sharp_panel_on(void);
extern bool g_panelOn;
extern bool update_screen_flag;
extern void asus_draw_last_screen(u32 img_color);
...
// ++ ASUS_BSP: Louis for backlight porting
#if 0
...
static int msm_fb_suspend_sub(struct msm_fb_data_type *mfd)
{
    ...
    //++ ASUS_BSP:Louis
    if (mfd->panel_info.type == MIPI_CMD_PANEL && !mfd->asus_panel_disable) {//Mickey+++, don't draw when plug into pad
        printk("[Display] Send black pattern before suspend fb0\n");
        asus_draw_last_screen(0x0);
    }
    //-- ASUS_BSP:Louis
...
static void msmfb_early_suspend(struct early_suspend *h)
{
	...
	*/
    ...
    {
        g_fb0_dsi_block = 1;
        printk("%s : asus_panel_disable=%d, g_fb0_on=%d, g_earlySuspend=%d, g_fb0_dsi_block(%d)+++\n",__func__,mfd->asus_panel_disable,g_fb0_on,g_earlySuspend, g_fb0_dsi_block);
...
//Mickey+++, add for DDS architecture
    if (panel_info->asus_dds_xres != 0 && panel_info->asus_dds_yres != 0)
    {
#ifdef CONFIG_FB_MSM_TRIPLE_BUFFER
        fix->smem_len = roundup(ALIGN(panel_info->asus_dds_xres,32) * panel_info->asus_dds_yres * 4 * 3, 0x10000);
#else
        fix->smem_len = roundup(ALIGN(panel_info->asus_dds_xres,32) * panel_info->asus_dds_yres * 4 * 2, 0x10000);
#endif
        mfd->fb_page = fix->smem_len / fix->line_length;
...
//+++ ASUS_BSP:Louis
    ...
    if ((!g_panelOn && info->node==0) && (g_A68_hwID < A80_EVB))//only for fb0
        sharp_panel_on();
#endif
//--- ASUS_BSP:Louis
	return 0;
...
unsigned long long int mdp4_overlay_get_vsync(struct fb_info *info);//Mickey+++, add for vsync ioctl
static int msm_fb_ioctl(struct fb_info *info, unsigned int cmd,
			unsigned long arg)
{
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)info->par;
    ...
    case MSMFB_GET_VSYNC:
        {
            unsigned long long int vsync = mdp4_overlay_get_vsync(info);
