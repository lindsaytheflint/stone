...
//#define VSYNC_EXPIRE_TICK 0 //ASUS_BSP: joe1_++ : Improve fps
#define VSYNC_EXPIRE_TICK 2 //ASUS_BSP: joe1_++ : Improve fps
...
static void mdp_dsi_clk_onoff(bool bOn)
{
    struct vsycn_ctrl *vctrl;
    vctrl = &vsync_ctrl_db[0];

    if(bOn) {
        printk("[Display] %s: SET_CLK_ON\n", __func__);
        mipi_dsi_clk_cfg(1);
        mdp_clk_ctrl(1);
        vctrl->clk_enabled = 1;
        ...
        printk("[Display] %s: SET_CLK_OFF\n", __func__);
        vsync_irq_disable(INTR_PRIMARY_RDPTR, MDP_PRIM_RDPTR_TERM);
        mipi_dsi_clk_cfg(0);
        mdp_clk_ctrl(0);
        vctrl->clk_enabled = 0; //make sure clk set 0 to end while loop
        vctrl->expire_tick = 0;
    }
}

void asus_mdp_dsi_clk_ctrl(struct fb_info *info, int enable)
{
    struct vsycn_ctrl *vctrl;
    struct mdp4_overlay_pipe *pipe;
    int cndx = 0;
    ...
    if (enable) {
        ...
        if (vctrl->clk_enabled == 1) {
            if (work_pending(&vctrl->clk_work)) {
                mutex_unlock(&vctrl->update_lock);
                return;
            }
            ...
            printk("[Display] %s: ov_koff=%d ov_done=%d dmap_koff=%d dmap_done=%d cpu=%d\n",
...
EXPORT_SYMBOL(asus_mdp_dsi_clk_ctrl);
//ASUS_BSP: Louis --
...
static void primary_rdptr_isr(int cndx)
{
    ...
    if (!update_screen_flag) //ASUS_BSP:Louis
	    schedule_work(&vctrl->vsync_work);
...
void mdp4_overlay0_done_dsi_cmd(int cndx)
{
    ...
    //Mickey+++, double check to prevent system hang
    if (pipe == NULL)
        return;
    //Mickey---
...
void mdp4_dsi_cmd_overlay(struct msm_fb_data_type *mfd)
{
	...
	struct mdp4_overlay_pipe *pipe;
	unsigned long flags;
//    long long tick; //ASUS_BSP: joe1_++ : Improve fps
