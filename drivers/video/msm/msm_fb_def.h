...
//ASUS_BSP: joe1_++: if the QSEED_TABLE is modified, print the messages for debug
#if 1
#define outpdw(port, val)  do{if( (((unsigned int)(port)&0x000fffff) == 0x00028100) || (((unsigned int)(port)&0x000fffff) == 0x00038100) )pr_err("[QSEED]: update 0x%x with 0x%x at func %s\n", (unsigned int)(port), (unsigned int)(val), __func__); writel(val, port);}while (0)
#else
