...
extern bool MHL_PP_Mode;//Mickey+++
extern bool MHL_HDCP_Mode;
...
//Mickey+++, skip HD resolution when MHL dongle is not support
	if (!MHL_PP_Mode && (video_format == HDMI_VFRMT_1920x1080p50_16_9 || video_format == HDMI_VFRMT_1920x1080p60_16_9))		
	{
	      return;
	}
	...
	uint32 num_og_cea_blocks)
{
	...
	if (svd != NULL) {
		...
		for (i = 0; i < len; ++i, ++svd) {
            ...
            //Mickey+++, for TV case, we ignore video format not in VESA spec
            if ( (video_format >= HDMI_VESA_MAX) && (video_format <= HDMI_VESA_MAX+1) )
                continue;
            //Mickey---
...
//Mickey+++, add for DDS
    ...
    {
        if (g_p03State == ASUS_P03_LOW)
            add_supported_video_format(&external_common_state->disp_mode_list,HDMI_VFRMT_1280x800p60_16_10);
        else if (g_p03State == ASUS_P03_HIGH)
            add_supported_video_format(&external_common_state->disp_mode_list,HDMI_VFRMT_1920x1080p60_16_9);
        else
        {
	      add_supported_video_format(&external_common_state->disp_mode_list,HDMI_VFRMT_1280x720p60_16_9);
        }
        external_common_state->hdmi_sink = TRUE;
    }
//Mickey---
