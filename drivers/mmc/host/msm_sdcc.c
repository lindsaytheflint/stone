...
//ASUS_BSP +++ Josh_Liao "disable erase to make factory reset work to hynix old emmc"
//Only use TRIM cmd, not use ERASE. They are both erase cmd. But erase looks like not work on hynix old emmc
...
/* Sometimes card doesn't response to host after resume. The log looks like:
 * mmc0: error -110 during resume (card was removed?)  -110 is timeout error.
 * And factory reset will become easy to fail. To avoid the bug, the workaround is to make card not enter sleep mode 
 * during suspend.
 */
