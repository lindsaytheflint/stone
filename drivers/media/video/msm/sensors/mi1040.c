				...
				int update_type, int res)
{
 ...
 //      struct sensor_info info;
	...
	//pr_err("%s +++ iCatch_first_open=%d\n",__func__, iCatch_first_open);       
	...
	if (update_type == MSM_SENSOR_REG_INIT) {
		...
		msm_sensor_write_res_settings(s_ctrl, res);
           ...
           if(res == MSM_SENSOR_RES_FULL){   
                pr_info("%s capture mode\n",__func__);
                sensor_write_reg(imx091_s_ctrl.sensor_i2c_client->client, 0x71EB, 0x01);//AE/AWB lock                           
            }else if(res == MSM_SENSOR_RES_QTR){
                pr_info("%s preview mode\n",__func__);  
                sensor_write_reg(imx091_s_ctrl.sensor_i2c_client->client, 0x71EB, 0x02);//AE/AWB unlock
            }else{
    		pr_info("[ISP]ERROR invalid res(%d)\n", res);
    		rc = -1;
            }
#endif                  
	}
...
#if 0
int32_t mi1040_sensor_set_sensor_mode(struct msm_sensor_ctrl_t *s_ctrl,
	int mode, int res)
{
	int32_t rc = 0;
...
static int mi1040_power_down(const struct msm_camera_sensor_info *data)
{
	...
	switch (g_A68_hwID)
	{
	    ...
	    //PMIC regulator - ISP 1.8V OFF
	       ...
	       rc = pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(imx091_s_ctrl.sensordata->sensor_platform_info->isp_suspend), &pm_isp_gpio_low);
	       if (rc != 0){
				pr_err("%s: isp_suspend failed\n", __func__);
	       }     	
	       ...
	       gpio_free(PM8921_GPIO_PM_TO_SYS(imx091_s_ctrl.sensordata->sensor_platform_info->isp_suspend));

	    //Turn off 24M CLK 	
	       //mi1040_sensor_probe_off(&imx091_s_ctrl.sensor_i2c_client->client->dev);

		//mutex_unlock(imx091_s_ctrl.msm_sensor_mutex);    //ASUS_BSP Stimber "Fix the issue which fail to re-open camera"
		   //msleep(1000);
...
static int mi1040_power_up(const struct msm_camera_sensor_info *data)
{
	...
	switch (g_A68_hwID)
	{
		...
		// ISP power on +++
		//ISP SUSPEND high
			//gpio_direction_output(imx091_s_ctrl.sensordata->sensor_platform_info->isp_suspend, 1);
			rc = pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(imx091_s_ctrl.sensordata->sensor_platform_info->isp_suspend), &pm_isp_gpio_high);
		    if (rc != 0){
				pr_err("%s: isp_suspend high failed\n", __func__);
		    }
		       
		//PMIC regulator - ISP 1.8V ON    
		...
		//ISP SUSPEND low
		    ...
		    rc = pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(imx091_s_ctrl.sensordata->sensor_platform_info->isp_suspend), &pm_isp_gpio_low);
		    if (rc != 0){
				pr_err("%s: isp_suspend low failed\n", __func__);
		    }	
		// ISP power on ---
		...
		// Wait for I2C ready
		    ...
		    pr_info("gpio ISP_SUSPEND(%d)\n",gpio_get_value(PM8921_GPIO_PM_TO_SYS(imx091_s_ctrl.sensordata->sensor_platform_info->isp_suspend)));
