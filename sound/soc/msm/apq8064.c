...
//Rice
#define MSM_CDC_PAMP PM8921_GPIO_PM_TO_SYS(18)
//static u32 top_spk_pamp_gpio  = PM8921_GPIO_PM_TO_SYS(18);
//static u32 bottom_spk_pamp_gpio = PM8921_GPIO_PM_TO_SYS(19);      
static int msm8960_pamp_on;
static int P03_pamp_on; //Rice
extern int IsA68Spk;	//Rice
//Rice
static int msm_spk_control;
//static int msm_ext_bottom_spk_pamp; //Rice
//static int msm_ext_top_spk_pamp; //Rice
...
*/
		if (spk_amp_gpio == MSM_CDC_PAMP) {

		ret = gpio_request(MSM_CDC_PAMP, "CDC PAMP");
		if (ret) {
			printk("%s: Error requesting CDC PAMP GPIO %u\n",
				__func__, MSM_CDC_PAMP);
			return;
		}

		ret = pm8xxx_gpio_config(MSM_CDC_PAMP, &param);
		if (ret)
			printk("%s: Failed to configure Spk Ampl"
				" gpio %u\n", __func__, MSM_CDC_PAMP);
		else {
			printk("%s: enable spkr amp gpio\n", __func__);
...
//Rice
	...
	{	
        ...
        while (iRetryCount) 
        {
            if((AX_MicroP_setGPIOOutputPin(OUT_uP_SPK_EN, 1))==0)
            {
                P03_pamp_on = 1;
                break;
            }
	...
	else
	{
		if (msm8960_pamp_on)
		{
			printk("%s: msm8960_pamp_on is already on\n", __func__);\
			return;
		}
		printk("%s: enable spkr amp\n", __func__);
		msm_enable_ext_spk_amp_gpio(MSM_CDC_PAMP);
        ApplyA68SPKGain();
		msm8960_pamp_on = 1;
	}
//Rice
...
//Rice
	...
	else
	{
		if (!msm8960_pamp_on)
		{
			printk("%s: msm8960_pamp_on is already off\n", __func__);\
			return;
		}
	...
	struct snd_kcontrol *k, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
        msm_ext_spk_power_amp_on(0);//Rice 
//Rice
#if 0 
...
//Rice
        ...
        msm_ext_spk_power_amp_off(0);//Rice
//Rice
#if 0 
		if (!strncmp(w->name, "Ext Spk Bottom Pos", 18))
...
//Rice
//HANS+++
static int msm_configure_headset_key_gpios(void)
{
	int ret;
	struct pm_gpio param = {
		.direction      = PM_GPIO_DIR_OUT,
	...
	ret = gpio_request(PM8921_GPIO_PM_TO_SYS(19), "MIC2_BIAS_EN");
	if (ret) {
		pr_err("%s: Failed to request gpio %d\n", __func__,
			PM8921_GPIO_PM_TO_SYS(19));
		return ret;
	}
	ret = pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(19), &param);
	if (ret){
		pr_err("%s: Failed to configure gpio %d\n", __func__,
			PM8921_GPIO_PM_TO_SYS(19));
	}
	...
	ret = gpio_request(PM8921_GPIO_PM_TO_SYS(20), "HS_PATH_EN");
	if (ret) {
		pr_err("%s: Failed to request gpio %d\n", __func__,
			PM8921_GPIO_PM_TO_SYS(20));
		return ret;
	}
	ret = pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(20), &param);
	if (ret){
		pr_err("%s: Failed to configure gpio %d\n", __func__,
			PM8921_GPIO_PM_TO_SYS(20));
	}
	else{
		gpio_direction_output(PM8921_GPIO_PM_TO_SYS(20), 0);
	}
	
	ret = gpio_request(6, "JACK_IN_DET");
	if (ret){
		pr_err("%s: Error requesting GPIO %d\n", __func__,6);
		gpio_free(6);
	}
	...
	ret = gpio_request(62, "HS_HOOK_DET");
	if (ret){
...
static void msm_free_headset_key_gpios(void)
{
	if (msm_headset_gpios_configured) {
		gpio_free(PM8921_GPIO_PM_TO_SYS(19));
...
//HANS++
	...
	if (msm_configure_headset_key_gpios()) {
		pr_err("%s Fail to configure headset key gpios\n", __func__);
		msm_headset_gpios_configured = 0;
	} else
		msm_headset_gpios_configured = 1;
//HANS--
...
static void __exit msm_audio_exit(void)
{
	...
	//msm_free_headset_mic_gpios();//Rice
//HANS++
	msm_free_headset_key_gpios();
//HANS--
