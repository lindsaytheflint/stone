...
#endif
//Bruno++ for P01
extern void ApplyA68SPKGain(void); 
...
/*
    ...
    {
        printk("P03_pamp_on is already %d\n", P01_SPK_control);
        return 0;
    }
    ...
    while (iRetryCount) 
    {
        if (P01_SPK_control)
        {
            ...
            ret = AX_MicroP_setGPIOOutputPin(OUT_uP_SPK_EN, 0);
            msleep(10);
        }
        if (!ret)
            break;
...
#endif
//Bruno++ for P01
//Rice:add for A68 speaker
static int A68_SPK_control;
static const char *A68_function[] = {"Off", "On"};
static const struct soc_enum A68_enum[] = {
    SOC_ENUM_SINGLE_EXT(2, A68_function),
};
static int A68_get_spk(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    printk("%s: A68_SPK_control = %d", __func__, A68_SPK_control);
    ucontrol->value.integer.value[0] = A68_SPK_control;
    return 0;
}

static int A68_set_spk(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{

    IsA68Spk = 1;
/*
    if (A68_SPK_control == ucontrol->value.integer.value[0])
    {
        printk("msm8960_pamp_on is already %d\n", A68_SPK_control);
        return 0;
    }
    A68_SPK_control = ucontrol->value.integer.value[0];
...
//Bruno++
#ifdef CONFIG_EEPROM_NUVOTON
static const struct snd_kcontrol_new A68_controls[] = {
    SOC_ENUM_EXT("A68 Speaker",     A68_enum[0], A68_get_spk,   A68_set_spk),
};
#endif
...
#endif
    snd_soc_add_platform_controls(platform, A68_controls, ARRAY_SIZE(A68_controls));    
//Bruno++
