		...
		if (count <= VOIP_MAX_VOC_PKT_SIZE) {
			...
			buf_node =
				list_first_entry(&prtd->free_in_queue,
						struct voip_buf_node, list);
			list_del(&buf_node->list);
			spin_unlock_irqrestore(&prtd->dsp_lock, dsp_flags);//Rice:fix viber issue
			if (prtd->mode == MODE_PCM) {
				...
				ret = copy_from_user(&buf_node->frame,
							buf, count);
			spin_lock_irqsave(&prtd->dsp_lock, dsp_flags);//Rice:fix viber issue
	...
	if (ret > 0) {
		...
		if (count <= VOIP_MAX_VOC_PKT_SIZE) {
			...
			buf_node = list_first_entry(&prtd->out_queue,
					struct voip_buf_node, list);
			list_del(&buf_node->list);
			spin_unlock_irqrestore(&prtd->dsp_ul_lock, dsp_flags);//Rice:fix viber issue
			if (prtd->mode == MODE_PCM)
			...
			spin_lock_irqsave(&prtd->dsp_ul_lock, dsp_flags);//Rice:fix viber issue
