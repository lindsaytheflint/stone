...
//ASUS_BSP+++ BennyCheng "extend restart timeout from 1 minute to 2 minute"
#define MDM_BOOT_TIMEOUT	120000L
//ASUS_BSP--- BennyCheng "extend restart timeout from 1 minute to 2 minute"
...
//ASUS_BSP+++ BennyCheng "add mdm ready check for ATD test"
static bool is_mdm_ready = 0;
//ASUS_BSP--- BennyCheng "add mdm ready check for ATD test"
...
//ASUS_BSP+++ BennyCheng "add mdm ready check for ATD test"
static int mdm_modem_ready_set(void *data, u64 val)
{
	return -ENOSYS;
}

static int mdm_modem_ready_get(void *data, u64 *val)
{
	*val = is_mdm_ready;
	return 0;
}

DEFINE_SIMPLE_ATTRIBUTE(mdm_modem_ready_fops,
			mdm_modem_ready_get,
			mdm_modem_ready_set, "%llu\n");
//ASUS_BSP--- BennyCheng "add mdm ready check for ATD test"
...
static int mdm_debugfs_init(void)
{
	...
	//ASUS_BSP+++ BennyCheng "add mdm ready check for ATD test"
	debugfs_create_file("mdm_ready", 0777, dent, NULL,
			&mdm_modem_ready_fops);
	//ASUS_BSP--- BennyCheng "add mdm ready check for ATD test"
