...
//extern void asus_draw_last_screen(u32);
...
// ASUS_BSP+++ Wenli "config subsystem restart"
...
static int is_subsystem_restart_enable(const char *sub_sys)
{
	int i;
	int temp_level = RESET_SOC;
	if (restart_level == RESET_SUBSYS_INDEPENDENT)
	{
		for (i = 0; i < sizeof(subsystem_name) / sizeof(subsystem_name[0]); ++i)
		{
			if ( sub_mask & (0x01 << i) )
			{
				...
				{
					temp_level = RESET_SUBSYS_INDEPENDENT;
					break;
				}
			}
		}
...
static void __subsystem_restart(struct subsys_data *subsys)
{
	...
	pr_err("start subsystem restart %s [level=%d]!\n", subsys->name,
				restart_level);
...
{
	struct subsys_data *subsys;
	int real_restart_level = restart_level;
...
// ASUS_BSP+++ Wenli "config subsystem restart"
	...
	real_restart_level = is_subsystem_restart_enable(subsys_name);
// ASUS_BSP--- Wenli "config subsystem restart"
#ifndef ASUS_SHIP_BUILD
	if (RESET_SOC == real_restart_level && !strcmp(subsys_name,"external_modem"))
	{
		pr_info("RED screen\n");
		preempt_disable();
		local_irq_disable();
		...
		flush_cache_all();
		//asus_draw_last_screen(0xFF0000FF);
		mdelay(3000);
	}
#endif
