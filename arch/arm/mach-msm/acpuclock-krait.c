...
#define HFPLL_MIN_VDD     700000 //LF: From 800000 / 700000
#define HFPLL_MAX_VDD    1350000 //LF: From 1350000 / 1450000
...
void acpuclk_set_vdd(unsigned int khz, int vdd_uv) {
  ...
  pr_warn("StoneK: user voltage table modified!\n");
  mutex_unlock(&driver_lock);
}
...
ssize_t acpuclk_set_vdd2(char *buf) 
{
	...
	for (i = 0; i < ARRAY_SIZE(size_cur); i++) {
		...
		if (cur_volt > HFPLL_MAX_VDD) {
			pr_info("Voltage Control: new volt is %d and its higher than %d so we set it to MAX_VDD(%d).\n", cur_volt, HFPLL_MAX_VDD, HFPLL_MAX_VDD);
			cur_volt = HFPLL_MAX_VDD;
		} else if (cur_volt < HFPLL_MIN_VDD) {
			pr_info("Voltage Control: new volt is %d and its lower than %d so we set it to MIN_VDD(%d).\n", cur_volt, HFPLL_MIN_VDD, HFPLL_MIN_VDD);
			cur_volt = HFPLL_MIN_VDD;
		}	

		drv.acpu_freq_tbl[i].vdd_core = cur_volt;  //LF: Removed *1000
