...
enum {
	...
	MSM_PM_DEBUG_HOTPLUG = BIT(8),
        MSM_PM_DEBUG_GPIO = BIT(9),     //Ledger
};
...
{
        if (msm_pm_debug_mask & MSM_PM_DEBUG_GPIO)
	{
        	static int gpio_n;
        	for(gpio_n=0;gpio_n<NR_GPIOS;gpio_n++)
        	{
               		if ( __raw_readl(GPIO_INTR_CFG(gpio_n))&0x9 )
               		{
                       	...
                       	printk("GPIO%d CFG:%x, IO:%x \n" ,gpio_n,__raw_readl(GPIO_CONFIG(gpio_n)),__raw_readl(GPIO_IN_OUT(gpio_n)));
		}
	}
        else 
                return;
...
static int msm_pm_enter(suspend_state_t state)
{
	...
	int64_t elapsed_time64 = msm_timer_get_sclk_time(&period);	//Ledger
	unsigned int elapsed_time;	//Ledger
...
//Ledger++
		if (elapsed_time64 != 0) {
			int64_t end_time = msm_timer_get_sclk_time(NULL);
			if (end_time != 0) {
				elapsed_time64 = end_time - elapsed_time64;
				if (elapsed_time64 < 0)
					elapsed_time64 += period;
			} else
				elapsed_time64 = 0;
		}
